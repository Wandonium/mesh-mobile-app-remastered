...
import { View, Dimensions, Animated, Platform, StatusBar } from 'react-native'

import Animator from './Animator'

const hasNotchAndroid = Platform.OS === 'android' && (hasNotch() || StatusBar.currentHeight > 24)
const SCREEN_HEIGHT = hasNotchAndroid
...
const calculateUpPosition = (screenHeight, containerHeight, offset) => {
  return {
    x: 0,
...
const calculateDownPosition = (upPosition, downDisplay) => {
  return {
    x: 0,
...
export default class BottomDrawer extends Component {
  ...
  constructor(props) {
     ...
     */
    this.UP_POSITION = calculateUpPosition(SCREEN_HEIGHT, this.props.containerHeight, this.props.offset)
    this.DOWN_POSITION = calculateDownPosition(this.UP_POSITION, this.DOWN_DISPLAY)
  ...
  toggleDrawerState() {
    ...
    // })
    this.setState((state) => {
      return { currentState: state.currentState === UP_STATE ? DOWN_STATE : UP_STATE }
    })
  }
  ...
  render() {
    return (
      <Animator
        ...
        <View
          style={{
            //height: hasNotchAndroid ? Math.sqrt(SCREEN_HEIGHT - StatusBar.currentHeight) : Math.sqrt(SCREEN_HEIGHT),
            backgroundColor: this.props.backgroundColor,
          }}
        />
      </Animator>
