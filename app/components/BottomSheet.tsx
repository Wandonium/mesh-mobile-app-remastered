...
export default class BottomSheetBehavior extends React.Component<Props, State> {
  ...
  constructor(props: Props) {
    ...
    for (let i = 1; i < snapPoints.length; i++) {
      const tuple: [Animated.Node<number>, Animated.Node<number>] = [add(snapPoints[i - 1], 10), sub(snapPoints[i], 25)]
    ...
    const tossForMaster =
      props.springConfig.hasOwnProperty('toss') && props.springConfig.toss != undefined ? props.springConfig.toss : toss
      ...
      i + 1 === snapPoints.length
        ...
        : cond(
            ...
            cond(
              ...
              cond(lessThan(destinationPoint, middlesOfSnapPoints[i][1]), snapPoints[i + 1], currentSnapPoint(i + 1)),
              snapPoints[i],
            ),
            cond(
              greaterThan(destinationPoint, middlesOfSnapPoints[i][1]),
              cond(lessThan(destinationPoint, middlesOfSnapPoints[i][0]), snapPoints[i + 1], currentSnapPoint(i + 1)),
              snapPoints[i],
            ),
          )
  ...
  private withEnhancedLimits(val: Animated.Node<number>, masterOffseted: Animated.Value<number>) {
    ...
    return block([
      ...
      cond(
        ...
        [set(prev, val), set(flagWasRunSpring, 0), stopClock(this.masterClockForOverscroll), set(wasRunMaster, 0)],
        [set(limitedVal, add(limitedVal, sub(val, prev))), cond(lessThan(limitedVal, min), set(limitedVal, min))],
      ),
      ...
      cond(
        ...
        [
          ...
          cond(
            or(eq(this.panState, GestureState.ACTIVE), eq(this.panMasterState, GestureState.ACTIVE)),
        ...
        <Animated.View
          ...
          <View
            ...
            {this.props.callbackNode && (
              <Animated.Code
                exec={onChange(
                  ...
                  set(
                    this.props.callbackNode,
                    divide(this.translateMaster, this.state.snapPoints[this.state.snapPoints.length - 1]),
                  ),
                )}
              />
            )}
            ...
            {this.props.contentPosition && (
              <Animated.Code exec={onChange(this.Y, set(this.props.contentPosition, sub(0, this.Y)))} />
            )}
